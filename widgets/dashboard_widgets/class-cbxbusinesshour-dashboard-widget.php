...
class CBXBusinessHoursDashboard {
	...
	public function CBXBusinessHours_dashboard_widget() {
		...
		$dashboard_widget = intval($setting->get_option('dashboard_widget', 'cbxbusinesshours_settings', 1));
		if($dashboard_widget == 0) return;

		//check user role for dashboard widget
		$dashboard_widget_roles = $setting->get_option('dashboard_widget_roles', 'cbxbusinesshours_settings', array());
		$current_user_roles = CBXBusinessHoursHelper::get_current_user_roles();
		$role_check = array_intersect($current_user_roles, $dashboard_widget_roles);
		if(sizeof($role_check) == 0 ) return ;
	...
	public function widget_display() {
		...
		$start_of_week = isset( $options['start_of_week'] ) ? intval( $options['start_of_week'] ) : $start_of_week_global;
	...
	public function widget_configure() {
		...
		if ( isset( $_POST['submit'] ) ) {
			...
			$options['start_of_week'] = isset( $_POST['cbxbusinesshours_start_of_week'] ) ? intval( $_POST['cbxbusinesshours_start_of_week'] ) : $start_of_week_global;
        ...
        <p>
            <label for="cbxbusinesshours_start_of_week"><?php echo esc_html__( 'Start of the Week:', 'cbxbusinesshours' ) ?></label>
            <select name="cbxbusinesshours_start_of_week" id="cbxbusinesshours_start_of_week">
				<?php
